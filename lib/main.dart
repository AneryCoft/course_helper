...
class _MainPageState extends State<MainPage> {
  ...
  Future<void> _checkUpdate() async {
    try {
      ...
      final response = await dio.get(
        'https://api.github.com/repos/AneryCoft/course_helper/releases/latest',
      );
      ...
      if (_isNewerVersion(latestVersion, currentVersion)) {
        _showUpdateDialog(
          ...
          downloadUrl:
              data['html_url'] ??
              'https://github.com/AneryCoft/course_helper/releases/latest',
        );
      }
    } catch (e) {
      // 忽略更新检查错误
  ...
  void _showSponsorDialog() {
    final hasSponsored =
        StorageManager.prefs.getBool(_hasSponsoredKey) ?? false;
  ...
  Future<void> _saveImageAndSponsor(BuildContext context) async {
    try {
      final imageData = await DefaultAssetBundle.of(
        context,
      ).load('images/mm_reward_qrcode.png');
      final bytes = imageData.buffer.asUint8List();

      await PhotoManager.editor.saveImage(
        bytes,
        filename: 'mm_reward_qrcode.png',
      );
      ...
      if (context.mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('赞赏码已保存到相册，感谢您的支持！')));
      }
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(
